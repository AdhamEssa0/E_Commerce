...
namespace E_Commerce.Infrastracture.Repository
{
    public class BasketRepository : IBasketRepository
    {
        ...
        public async Task<CustomerBasket> CreateOrUpdateBasAsync(CustomerBasket basket, TimeSpan? TimeLive = null, CancellationToken ct = default)
        {
            var value = JsonSerializer.Serialize(basket);
            var result = await _database.StringSetAsync(basket.Id, value, TimeLive ?? TimeSpan.FromDays(7));
